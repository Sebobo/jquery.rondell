...
task 'watch', 'Watch sass and src folders for changes and recompile from src and scss folders to lib and css', ->
  ...
  coffeeProcess = spawn 'coffee', ['--bare', '--join', 'lib/jquery.rondell.js', '--watch', '--compile', 'src/']
...
task 'minify', 'Minify the plugins *.js and *.css before release', (options) ->
  ...
  exec "java -jar \"/Applications/yuicompressor.jar\" -o lib/jquery.rondell-#{options.version}.min.js lib/jquery.rondell.js", (err, stdout, stderr) ->
    throw err if err
    console.log stdout + stderr
    console.log "Created minified js file"
    
  exec "sass --style compressed scss/jquery.rondell.scss css/jquery.rondell-#{options.version}.min.css", (err, stdout, stderr) ->
    throw err if err
    console.log stdout + stderr
